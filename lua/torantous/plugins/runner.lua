...
Runner.config = {
  commands = {
    ...
    cpp = {
      ...
      -- DEFAULT (Clang 18+, C++23)
      build = "clang++ -Wall -Wextra -std=c++23 -g %a %f -o %b",
      run = "clang++ -Wall -Wextra -std=c++23 -g %f -o %b && ./%b %a",        -- compile-on-run
      ...
      -- ── RED TEAM: Windows Cross-Compile (WSL → .exe) ──
      -- Requires: sudo apt install g++-mingw-w64-x86-64
      -- Generates a statically-linked PE32+ executable from Linux.
      win32 = "x86_64-w64-mingw32-g++ -static %a %f -o %b.exe",

      -- ── RED TEAM: DLL Generation (WSL → .dll) ──
      -- Generates a Windows shared library for DLL injection / side-loading.
      -- Mark exported functions with __declspec(dllexport) or a .def file.
      dll = "x86_64-w64-mingw32-g++ -shared -static %a %f -o %b.dll",

      -- ── RED TEAM: 32-bit Windows EXE (WoW64 targets) ──
      win32_x86 = "i686-w64-mingw32-g++ -static %a %f -o %b_x86.exe",

      -- ── RED TEAM: Assembly Dump (objdump, Intel syntax) ──
      -- Inspect compiled output to verify compiler behaviour, ROP gadget search, etc.
...
local function get_file_info()
  ...
  if not bufname or bufname == "" then
    local ext_map = {
      cpp = "cpp", c = "c", python = "py", lua = "lua",
      go = "go", rust = "rs", asm = "asm", java = "java",
      javascript = "js", typescript = "ts", sh = "sh",
      bash = "sh", ruby = "rb", php = "php",
    }
...
local function create_term_window(cmd, cwd, env_tbl)
  ...
  if env_tbl then term_opts.env = env_tbl end
...
function Runner.run(mode, args, opts)
  ...
  if cmake_filetypes[ft] and has_cmake() then
    ft = "cmake"
    if mode == "build_run" then mode = "run" end
  end
  ...
  if env then
    local env_str = ""
    for k, v in pairs(env) do env_str = env_str .. k .. "=" .. v .. " " end
...
function Runner.show_info()
  ...
  local active = 0
  for _ in pairs(Runner.state.jobs) do active = active + 1 end
...
function Runner.pick_mode()
  ...
  table.sort(modes, function(a, b) return a.mode < b.mode end)
...
map("n", "<leader>rs", function() Runner.stop() end,               { desc = "Stop (current buf)" })
map("n", "<leader>rS", function() Runner.stop_all() end,           { desc = "Stop all" })
...
map("n", "<leader>rh", function() Runner.run_hex() end,            { desc = "Run + hex dump" })
map("n", "<leader>rn", function() Runner.run_interactive() end,    { desc = "Run interactive" })
map("n", "<leader>rd", function() Runner.debug() end,              { desc = "Debug (DAP)" })
map("n", "<leader>rw", function() Runner.toggle_watch() end,       { desc = "Toggle watch mode" })
